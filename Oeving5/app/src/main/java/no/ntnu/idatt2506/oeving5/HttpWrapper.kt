    ...
    String?): String {
        ...
        try {
            BufferedReader(InputStreamReader(inputStream, charset)).use {
                ...
                while (line != null) { // Byttet ut do-while med while
                    body += "$line\n" // og byttet om disse to linjene
                    line = bufferedReader.readLine() // for å unngå null på slutten
                }
            }
        } catch (e: Exception) {
    ...
    private fun getCharSet(connection: URLConnection): String? {
        ...
        for (param in contentInfo) {
            if (param.startsWith("charset=")) {
                charset = param.split("=").toTypedArray()[1]
            }
        }
